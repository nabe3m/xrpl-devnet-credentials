...
const CREDENTIAL_TYPE =
	process.env.CREDENTIAL_TYPE || "XRPLCommunityExamCertification";
...
async function revokeCredential() {
	...
	try {
		...
		if (RECIPIENT_ADDRESS) {
			console.log(`対象の受信者アドレス: ${RECIPIENT_ADDRESS}`);
		}
		...
		const txHash = await credentialService.revokeCredential(
			CREDENTIAL_TYPE,
			RECIPIENT_ADDRESS,
		);
